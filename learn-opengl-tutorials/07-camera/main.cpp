...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mode){
  ...
  /* this results in buggy camera movement */
...
void move(){
  // Camera controls
  // if we do this, we have different velocities in diferent hardware
  //GLfloat cameraSpeed = 0.05f;
  glm::vec3 noFlyCameraFront = glm::vec3(cameraFront.x, 0.0, cameraFront.z);
  GLfloat cameraSpeed = 5.0f * deltaTime;
  if(keys[GLFW_KEY_W])
    cameraPos += cameraSpeed * cameraFront;
    //cameraPos += cameraSpeed * noFlyCameraFront;
  if(keys[GLFW_KEY_S])
    cameraPos -= cameraSpeed * cameraFront;
    //cameraPos -= cameraSpeed * noFlyCameraFront;
  if(keys[GLFW_KEY_A])
    cameraPos -= glm::normalize(glm::cross(cameraFront, cameraUp)) * cameraSpeed;
    //cameraPos -= glm::normalize(glm::cross(noFlyCameraFront, cameraUp)) * cameraSpeed;
  if(keys[GLFW_KEY_D])
    cameraPos += glm::normalize(glm::cross(cameraFront, cameraUp)) * cameraSpeed;
    //cameraPos += glm::normalize(glm::cross(noFlyCameraFront, cameraUp)) * cameraSpeed;
  // no flying
...
int main(){
  ...
  GLFWwindow* window = glfwCreateWindow(800, 600, "06 coordinate systems", nullptr, nullptr);
  ...
  while (!glfwWindowShouldClose(window)){
    // calculate deltaTime so that every hardware yields the same velocity
